...
class Exp:
    def __init__(self, multi_handler):
        ...
        for group_id, tst_handlers in enumerate(multi_handler.tst_handlers_group):
            print(
                f"Test group {group_id}", list(map(lambda x: x.data_name, tst_handlers))
            )
        self.metrics = dict()
    ...
    def run(self):
        ...
        for ep in range(stloc, args.epoch):
            ...
            self.model.assign_experts(
                self.multi_handler.trn_handlers, reca=True, log_assignment=True
            )
            ...
            if tst_flag:
                for handler_group_id in range(
                    len(self.multi_handler.tst_handlers_group)
                ):
                    tst_handlers = self.multi_handler.tst_handlers_group[
                        handler_group_id
                    ]
                    self.model.assign_experts(
                        tst_handlers, reca=False, log_assignment=True
                    )
                    ...
                    log(
                        self.make_print(
                            "Test" + str(handler_group_id), ep, reses, tst_flag
                        )
                    )
                    if args.use_wandb:
                        wandb.log(
                            data={
                                f"Recall_dataset_{handler_group_id}": reses["Recall"],
            ...
            hours, minutes, seconds, milliseconds = get_human_like_epoch_run_time(
                epoch_elapesed_time
            )  ### LOG ###
            print(
                f"NOTICE: Epoch train run time = {hours}[h]::{minutes}[min]::{seconds}[s].{milliseconds}[ms]"
            )

        for test_group_id in range(len(self.multi_handler.tst_handlers_group)):
            repeat_times = 1
            overall_recall, overall_ndcg = np.zeros(repeat_times), np.zeros(
                repeat_times
            )
            overall_tstnum = 0
            ...
            for i, handler in enumerate(tst_handlers):
                for topk in [args.topk]:
                    ...
                    for _ in range(repeat_times):
                        ...
                        self.model.assign_experts(
                            [handler], reca=False, log_assignment=False
                        )
    ...
    def train_epoch(self):
        ...
        reassign_steps = sum(
            list(map(lambda x: x.reproj_steps, self.multi_handler.trn_handlers))
        )
        for i, batch_data in enumerate(trn_loader):
            ...
            if (counter[dataset_id] + 1) % self.multi_handler.trn_handlers[
                dataset_id
            ].reproj_steps == 0:
                self.multi_handler.trn_handlers[dataset_id].make_projectors()
            if (i + 1) % reassign_steps == 0:
                self.model.assign_experts(
                    self.multi_handler.trn_handlers, reca=True, log_assignment=False
                )
        ret = dict()
    ...
    def make_trn_masks(self, numpy_usrs, csr_mat):
        ...
        trn_masks = t.from_numpy(
            np.stack([trn_masks.row, trn_masks.col], axis=0)
        ).long()
    ...
    def test_epoch(self, handler, dataset_id):
        with t.no_grad():
            ...
            for i, batch_data in enumerate(tst_loader):
                ...
                trn_masks, cand_size = self.make_trn_masks(
                    batch_data.numpy(), tst_loader.dataset.csrmat
                )
                feats = handler.projectors
                all_preds = expert.pred_for_test(
                    (usrs, trn_masks),
                    cand_size,
                    feats,
                ...
                recall, ndcg = self.calc_recall_ndcg(
                    top_locs, tst_loader.dataset.tstLocs, usrs
                )
                ep_recall += recall
                ep_ndcg += ndcg
                log(
                    "Steps %d/%d: recall = %.2f, ndcg = %.2f          "
                    % (i, steps, recall, ndcg),
                    save=False,
                    oneline=True,
                )
